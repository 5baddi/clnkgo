        ...
        <div class="card card-md">
            ...
            <div class="card-body">
                ...
                <div class="text-center mt-4">
                    <a href="{{ ($currentPack && $currentPack->id === $pack->id) ? '#' : route('subscription.pack.billing', ['pack' => $pack->id]) }}" class="btn {{ ($currentPack && $currentPack->id === $pack->id) ? 'btn-dark' : '' }} w-100">{{ $currentPack && $currentPack->id === $pack->id && ($subscription->isActive() && ! $subscription->isTrial()) ? 'Current Plan' : 'Choose ' . ucwords($pack->name) }}</a>
                </div>
            </div>
        </div>
    </div>
