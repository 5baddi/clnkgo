    ...
    <div class="collapse navbar-collapse" id="navbar-menu">
      <ol class="breadcrumb breadcrumb-arrows" aria-label="breadcrumbs">
        <li class="breadcrumb-item">
            <a href="{{ route('journalist') }}">Journalist</a>
        </li>
        @foreach (request()->segments() as $key => $segment)
            @if ($segment === 'journalist')
            @continue
            @endif
            @if(! is_numeric($segment) && (preg_match('/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/', $segment) !== 1))
            <li class="breadcrumb-item {{ (sizeof(request()->segments()) - 1) === ($key) ? 'active' : '' }}"><a href="{{ url('journalist/' . $segment) }}">{{ ucfirst($segment) }}</a></li>
            @endif
        @endforeach
      </ol>
    </div>
